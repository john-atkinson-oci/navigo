...
angular.module('taskRunner').
    factory('templateService', function () {
        ...
        var requiredIcon = '<i ng-show="param.required" class="fa fa-asterisk" title="Required"></i>';
        ...
        var labelTpl = '<dl style="margin-bottom: 5px"><dt>' + advancedIcon + '{{param.label}}<dt><dd>{{param.desc}}</dd></dl>';
        ...
        var projection = '<div ng-hide="param.advanced && !show" class="process-control">' + labelTpl + projectionSelect + requiredIcon + errorTpl + '</div>';
        var stringChoice = '<div ng-hide="param.advanced && !show" class="process-control">' + labelTpl + selectTpl + requiredIcon + errorTpl + '</div>';
        var textArea = '<div ng-hide="param.advanced && !show" class="process-control">' + labelTpl + '<textarea ng-model="param.value" style="width: 96%"></textarea>' + requiredIcon + errorTpl + '</div>';
        var stringTpl = '<div ng-hide="param.advanced && !show" class="process-control">' + labelTpl + textInputTpl + requiredIcon + errorTpl + '</div>';
        var checkBox = '<div ng-hide="param.advanced && !show" class="process-control">' + '<dl style="margin-bottom: 5px"><dt>' + advancedIcon + '{{param.label}}<dt><dd><label style="font-weight: normal"><input type="checkbox" ng-model="param.value"> {{param.desc}}</label></dd></dl>' + requiredIcon + errorTpl + '</div>';
        var missing = '<div class="process-control" style="color:red">Missing {{param.name}} template</div>';
        var intTpl = '<div ng-hide="param.advanced && !show" class="process-control">' + labelTpl + intInputTpl + requiredIcon + errorTpl + '</div>';
