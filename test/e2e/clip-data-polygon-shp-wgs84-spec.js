...
describe('Clip Data by Polygon Task where output format is SHP and output projection is Web Mercator Auxiliary Sphere', function() {
    ...
    function _addFileTypeToQueue() {
        ...
        browser.get(server + '#/search?q=title:ca_ozone_pts&disp=71f42f58&view=card');
        Util.waitForSpinner();
    ...
    it('should load clip_data_by_polygon task and set output format to SHP and change projection', function() {
        ...
        // Set the output projection from Same As Input to WGS84
        selectChoiceElements.filter(function(elem) {
            return elem.getText().then(function(text) {
                return text === 'Same As Input';
            });
        }).click();
        var selectTextBox = element(by.css('.select2-input.select2-focused'));
        selectTextBox.sendKeys('WGS 1984 Web Mercator (auxiliary sphere)');
        selectTextBox.sendKeys(protractor.Key.ENTER);
